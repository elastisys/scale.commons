...
public class TestAuthTokenHeaderValidator {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestAuthTokenHeaderValidator.class);
    ...
    public void beforeTestMethod() throws Exception {
        ...
        AsymmetricKeyAuthTokenValidator tokenValidator = new AsymmetricKeyAuthTokenValidator(signatureKeyPair)
    ...
    public void validateMalformedHeader() {
        ...
        try {
            ...
            assertValidationException(e, "failed to validate Authorization token",
    ...
    public void validateValidAuthToken() throws Exception {
        ...
        String signedToken = signToken(TOKEN_ISSUER, signatureKeyPair, expirationTime);
        ...
        assertThat(claims.getIssuedAt(), is(NumericDate.fromMilliseconds(UtcTime.now().getMillis())));
    ...
    public void validateExpiredAuthToken() throws Exception {
        DateTime expirationTime = UtcTime.now().plusMinutes(10);
        String tokenWithExpiration = signToken(TOKEN_ISSUER, signatureKeyPair, expirationTime);
        ...
        try {
            ...
            assertValidationException(e, "failed to validate Authorization token", "no longer valid");
        }
    }
    ...
    public void failingAuthTokenValidator() throws Exception {
        ...
        String signedToken = signToken(TOKEN_ISSUER, signatureKeyPair, UtcTime.now().plusMinutes(10));

        try {
            validator.validate("Bearer " + signedToken);
            fail("validation should have failed");
        } catch (AuthTokenValidationException e) {
            assertValidationException(e, "failed to validate Authorization token", "internal error");
        }
    }
    ...
    private void assertValidationException(AuthTokenValidationException e, String expectedMessage,
            String detailContains) {
        assertThat(e.getMessage(), is(expectedMessage));
        assertTrue("error detail '" + e.getDetail() + "' did not contain '" + detailContains + "'",
