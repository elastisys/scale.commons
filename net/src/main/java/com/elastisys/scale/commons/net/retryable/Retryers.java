...
public class Retryers {
	 ...
	 */
	public static <R> Retryable<R> fixedDelayRetryer(String name,
			Callable<R> callable, int fixedDelay, TimeUnit unit, int maxAttempts) {

		return new Retryable<>(callable).name(name)
				.delay(fixed(fixedDelay, unit))
	 ...
	 */
	public static <R> Retryable<R> fixedDelayRetryer(String name,
			Callable<R> callable, int fixedDelay, TimeUnit unit,
			int maxAttempts, Predicate<R> responsePredicate) {

		return new Retryable<>(callable).name(name)
				.delay(fixed(fixedDelay, unit))
				.stop(afterAttempts(maxAttempts)).retryOnException()
				.retryUntilResponse(responsePredicate);
	}
	 ...
	 * @return The constructed {@link Retryable}.
	 */
	public static <R> Retryable<R> exponentialBackoffRetryer(String name,
			Callable<R> callable, int initialDelay, TimeUnit unit,
			int maxAttempts) {

		return new Retryable<>(callable).name(name)
				.delay(exponentialBackoff(initialDelay, unit))
				.stop(afterAttempts(maxAttempts)).retryOnException();
	}
