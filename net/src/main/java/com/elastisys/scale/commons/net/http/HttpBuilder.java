...
public class HttpBuilder {
     ...
     * @return
     */
    public HttpBuilder clientBasicAuth(BasicCredentials clientBasicCredentials) {
        CredentialsProvider credentialsProvider = new BasicCredentialsProvider();
        credentialsProvider.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials(
                clientBasicCredentials.getUsername(), clientBasicCredentials.getPassword()));
     ...
     * @return
     */
    public HttpBuilder clientCertAuth(CertificateCredentials clientCertCredentials) {
        ...
        try {
            keyStore = SslUtils.loadKeyStore(clientCertCredentials.getKeystoreType(), keystorePath, keystorePassword);
        } catch (Exception e) {
     ...
     * @return
     */
    public HttpBuilder clientCertAuth(KeyStore clientCertKeystore, String keyPassword) {
        checkArgument(clientCertKeystore != null, "null keystore given");
        checkArgument(keyPassword != null,
                "null keyPassword given (keystore keys cannot " + "be recovered without a password)");

        try {
            this.sslContextBuilder.clientAuthentication(clientCertKeystore, keyPassword);
        } catch (Exception e) {
            throw new HttpBuilderException("failed to set client certificate credentials: " + e.getMessage(), e);
        }
        return this;
    ...
    public HttpBuilder clientJwtAuth(String signedAuthToken) {
        return header(HttpHeaders.AUTHORIZATION, String.format("Bearer %s", signedAuthToken));
    }
    ...
    public HttpBuilder verifyHostCert(boolean shouldVerify) {
        try {
            ...
            throw new HttpBuilderException(
                    String.format("failed to set verifyHostCert to %s: %s", shouldVerify, e.getMessage()), e);
        }
    }
     ...
     */
    public HttpBuilder serverAuthTrustStore(KeyStoreType type, String trustStorePath, String storePassword)
            throws HttpBuilderException {
        KeyStore trustStore = null;
        try {
            trustStore = SslUtils.loadKeyStore(type, trustStorePath, storePassword);
        } catch (Exception e) {
     ...
     * @throws KeyStoreException
     */
    public HttpBuilder serverAuthTrustStore(KeyStore trustStore) throws HttpBuilderException {
        try {
    ...
    public HttpBuilder verifyHostname(boolean shouldVerify) {
        HostnameVerifier sslHostnameVerifier = shouldVerify ? new DefaultHostnameVerifier()
