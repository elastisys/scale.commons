...
public class TestRetryers {
	...
	public void testFixedDelayRetryer() throws Exception {
		...
		Retryable<Integer> retryer = Retryers.fixedDelayRetryer("counter",
				counter, 10, MILLISECONDS, 5);
		...
		retryer = Retryers.fixedDelayRetryer("counter", counter, 10,
				MILLISECONDS, 3);
		try {
			retryer.call();
	...
	public void testFixedDelayRetryerWithResultPredicate() throws Exception {
		...
		int maxRetries = 15;
		Retryable<Integer> retryer = Retryers.fixedDelayRetryer("counter",
				counter, 10, MILLISECONDS, maxRetries, equalTo(10));
	...
	public void testExponentialBackoffRetryer() throws Exception {
		...
		int initialDelay = 1;
		int maxRetries = 5;
		Retryable<Integer> retryer = Retryers.exponentialBackoffRetryer(
				"exp-counter", counter, initialDelay, MILLISECONDS, maxRetries);
		assertThat(retryer.call(), is(4));
		assertThat(retryer.getAttempts(), is(4));
		...
		maxRetries = 5;
		retryer = Retryers.exponentialBackoffRetryer("exp-counter", counter,
				initialDelay, MILLISECONDS, maxRetries);
		try {
			retryer.call();
			fail("should not succeed");
		} catch (GaveUpException e) {
	...
	@Test
	public void testExponentialBackoffRetryerWithResultPredicate()
			throws Exception {
		RuntimeException fault = new RuntimeException("failed!");
		...
		maxRetries = 8;
		retryer = Retryers.exponentialBackoffRetryer("exp-counter", counter,
				initialDelay, MILLISECONDS, maxRetries, equalTo(10));
		try {
			retryer.call();
			fail("should not succeed");
		} catch (GaveUpException e) {
