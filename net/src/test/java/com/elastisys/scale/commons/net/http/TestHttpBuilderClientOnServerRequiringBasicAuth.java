...
public class TestHttpBuilderClientOnServerRequiringBasicAuth {
    ...
    private static final BasicCredentials untrustedPasswordCredentials = new BasicCredentials("molly",
            "secretpassword");
    ...
    public void authenticateWithUntrustedUser() throws IOException {
        Http http = Http.builder().clientBasicAuth(untrustedPasswordCredentials).build();
        try {
            http.execute(new HttpGet(url("/")));
        } catch (HttpResponseException e) {
            assertThat(e.getStatusCode(), is(401));
    ...
    public void authenticateWithBasicAndCertCredentials() throws IOException {
        Http http = Http.builder().clientBasicAuth(trustedPasswordCredentials).clientCertAuth(certCredentials).build();
