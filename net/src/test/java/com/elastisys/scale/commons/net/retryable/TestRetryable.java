...
public class TestRetryable {
    ...
    public void testWithErrorSuppression() throws Exception {
        ...
        Retryable<String> retryable = new Retryable<>(task).retryOnError(Exception.class);
        String result = retryable.call();
        assertThat(result, is("hello world"));
    ...
    public void testWithErrorSuppressionThatDoesNotMatchException() throws Exception {
        Exception fault = new RuntimeException("task failed!");
        Callable<String> task = new FailNTimesAndReturn(5, fault, "hello world");

        // no IllegalArgumentException will be thrown
        Retryable<String> retryable = new Retryable<>(task).retryOnError(IllegalArgumentException.class);
        retryable.call();
    }
    ...
    public void testWithResponsePredicate() throws Exception {
        ...
        Retryable<Integer> retryable = new Retryable<>(task).retryUntilResponse(equalTo(1));
    ...
    public void testWithResponsePredicateAndErrorSuppression() throws Exception {
        IllegalStateException fault = new IllegalStateException("cannot handle low numbers!");

        // should raise 10 errors before starting to return count values
        Callable<Integer> task = new FailNTimesCounter(10, fault);
        Retryable<Integer> retryable = new Retryable<>(task).retryOnException().retryUntilResponse(equalTo(15));
    ...
    public void testWithStopStrategy() throws Exception {
        ...
        Callable<Integer> task = new FailNTimesCounter(10, fault);
        Retryable<Integer> retryable = new Retryable<>(task).retryOnException().retryUntilResponse(equalTo(15))
                .stop(afterAttempts(15));
        assertThat(retryable.call(), is(15));
        assertThat(retryable.getAttempts(), is(15));

        // stop strategy that quits after only 1 attempt
        task = new FailNTimesCounter(10, fault);
        retryable = new Retryable<>(task).retryOnException().retryUntilResponse(equalTo(15)).stop(afterAttempts(1));
        try {
            retryable.call();
        ...
        task = new FailNTimesCounter(10, fault);
        retryable = new Retryable<>(task).retryOnException().retryUntilResponse(equalTo(15)).stop(afterAttempts(10));
        try {
            retryable.call();
        } catch (GaveUpException e) {
            // expected
    ...
    public void testWithDelayStrategy() throws Exception {
        ...
        task = new Counter();
        retryable = new Retryable<>(task).retryUntilResponse(equalTo(2)).delay(fixed(20, MILLISECONDS));
        ...
        // delay should have been introduced
        task = new Counter();
        retryable = new Retryable<>(task).retryUntilResponse(equalTo(4)).delay(fixed(20, MILLISECONDS));
    ...
    public void testWithName() throws Exception {
        Callable<Integer> task = new Counter();
        Retryable<Integer> retryable = new Retryable<>(task).retryUntilResponse(equalTo(10)).name("await-counter=10");
    ...
    public void testCombinationWithExponentialBackoff() throws Exception {
        Exception fault = new IllegalStateException("can't handle too small values!");
        Callable<Integer> task = new FailNTimesCounter(5, fault);
        Retryable<Integer> retryable = new Retryable<>(task).retryOnException().retryUntilResponse(equalTo(10))
                .stop(afterTime(2, SECONDS)).delay(exponentialBackoff(1, MILLISECONDS)).name("backoff-counter");
        assertThat(retryable.call(), is(10));
        assertThat(retryable.getAttempts(), is(10));
    ...
    private static class FailNTimesAndReturn implements Callable<String> {
        ...
        public FailNTimesAndReturn(int plannedFailures, Exception failure, String returnValue) {
