...
/**
 * Exercises the response handling of the {@link Http} class. Requests with 2XX
 * responses should be accepted whereas requests with other response codes
 * should raise {@link HttpResponseException}s.
 */
public class TestHttpBuilderClientResponseHandling {
	...
	public void getRequestWith200Response() throws IOException {
		HttpRequestResponse response = client("user", "secret")
				.execute(new HttpGet(url("/hello")));
	...
	public void postRequestWith200Response() throws IOException {
		...
		postRequest.setEntity(
				new StringEntity("{\"a\": 1}", ContentType.APPLICATION_JSON));
		HttpRequestResponse response = client("user", "secret")
				.execute(postRequest);
		assertThat(response.getStatusCode(), is(200));
		assertThat(response.getResponseBody(), is("Hello World!"));
	...
	public void getRequestWith204ResponseAndNoMessageBody() throws IOException {
		HttpRequestResponse response = client("user", "secret")
				.execute(new HttpGet(url("/silent")));
	 ...
	 */
	@Test
	public void postRequestWith204ResponseAndNoMessageBody()
			throws IOException {
	...
	private Http client(String username, String password) {
		return Http.builder()
				.clientBasicAuth(new BasicCredentials(username, password))
				.build();
	}
}
