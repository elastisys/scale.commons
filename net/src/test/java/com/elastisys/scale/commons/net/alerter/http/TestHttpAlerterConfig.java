...
public class TestHttpAlerterConfig {
    ...
    public void basicSanity() {
        ...
        HttpAlerterConfig config = new HttpAlerterConfig(urls, severityFilter, auth, connectionTimeout, socketTimeout);
        assertThat(config.getDestinationUrls(), is(urls));
        assertThat(config.getSeverityFilter().getFilterExpression(), is(severityFilter));
    ...
    public void createWhenRelyingOnDefaults() {
        ...
        assertThat(config.getDestinationUrls(), is(urls));
        assertThat(config.getSeverityFilter().getFilterExpression(), is(HttpAlerterConfig.DEFAULT_SEVERITY_FILTER));
        assertThat(config.getAuth().getBasicCredentials().isPresent(), is(false));
        assertThat(config.getAuth().getCertificateCredentials().isPresent(), is(false));
        assertThat(config.getConnectTimeout(), is(HttpAlerterConfig.DEFAULT_CONNECTION_TIMEOUT));
        assertThat(config.getSocketTimeout(), is(HttpAlerterConfig.DEFAULT_SOCKET_TIMEOUT));
    }
