...
public class TestHttpBuilderClientOnServerRequiringCertAndBasicAuth {
    ...
    private static final BasicCredentials untrustedPasswordCredentials = new BasicCredentials("user", "badpassword");
    ...
    private static Server createHttpsServer(int httpsPort) {
        ServletDefinition servlet = new ServletDefinition.Builder().servlet(new HelloWorldServlet()).servletPath("/")
                .requireBasicAuth(true).realmFile(SERVER_REALM_FILE).requireRole("USER").build();
    ...
    public void correctCertAndPassword() throws IOException {
        Http http = Http.builder().clientBasicAuth(trustedPasswordCredentials).clientCertAuth(trustedClientJksCert)
                .build();
    ...
    public void badCertAndCorrectPassword() throws IOException {
        Http http = Http.builder().clientBasicAuth(trustedPasswordCredentials).clientCertAuth(untrustedClientPkcs12Cert)
                .build();
    ...
    public void correctCertAndBadPassword() throws IOException {
        Http http = Http.builder().clientBasicAuth(untrustedPasswordCredentials).clientCertAuth(trustedClientJksCert)
                .build();
