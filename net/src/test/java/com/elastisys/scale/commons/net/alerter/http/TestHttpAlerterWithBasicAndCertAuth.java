...
public class TestHttpAlerterWithBasicAndCertAuth {
    ...
    public void deliverAlert() {
        ...
        HttpAlerter certAlerter = new HttpAlerter(new HttpAlerterConfig(asList(webhookUrl()), ".*", certAuth), null);
        certAlerter.handleAlert(alert);
        assertThat(webhook.getReceivedMessages().isEmpty(), is(true));

        // ... then verify that we can deliver with both basic and cert
        // credentials
        HttpAuthConfig basicAndCertAuth = new HttpAuthConfig(new BasicCredentials("user", "secret"),
                new CertificateCredentials(KeyStoreType.PKCS12, TRUSTED_CLIENT_PKCS12_KEYSTORE,
                        TRUSTED_CLIENT_PKCS12_KEYSTORE_PASSWORD, TRUSTED_CLIENT_PKCS12_KEYSTORE_PASSWORD));
        HttpAlerter basicAndCertAlerter = new HttpAlerter(
                new HttpAlerterConfig(asList(webhookUrl()), ".*", basicAndCertAuth), null);
