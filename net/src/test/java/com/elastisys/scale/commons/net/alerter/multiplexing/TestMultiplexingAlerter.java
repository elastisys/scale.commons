...
public class TestMultiplexingAlerter {
    ...
    public void beforeTestMethod() {
        ...
        this.multiplexingAlerter = new MultiplexingAlerter(FilteringAlerter.TOPIC_IDENTITY_FUNCTION);
    ...
    public void registerHttpAlerterConfigWithoutStandardTags() {
        ...
        assertThat(this.multiplexingAlerter.alerters(),
                is(alerters(filtered(http(http1Config, standardTags), alertConfig.getDuplicateSuppression()))));
    }
    ...
    public void registerSmtpAlerterConfigWithoutStandardTags() {
        ...
        this.multiplexingAlerter.registerAlerters(alertConfig, standardTags);
        assertThat(this.multiplexingAlerter.alerters(),
                is(alerters(filtered(smtp(smtp1Conf, standardTags), alertConfig.getDuplicateSuppression()))));
    }
    ...
    public void registerHttpAndSmtpAlerterConfig() {
        ...
        assertThat(this.multiplexingAlerter.alerters(),
                is(alerters(filtered(smtp(smtp1Conf, standardTags()), alertConfig.getDuplicateSuppression()),
                        filtered(http(http1Conf, standardTags()), alertConfig.getDuplicateSuppression()))));
    }
    ...
    public void registerAdditionalAlerters() {
        ...
        SmtpAlerterConfig smtp1Conf = smtpConfig("john@doe.com", ".*");
        List<SmtpAlerterConfig> smtpAlerters = Arrays.asList(smtp1Conf);
        AlertersConfig alert1Config = alertConfig(smtpAlerters, null, new TimeInterval(10L, TimeUnit.MINUTES));
        this.multiplexingAlerter.registerAlerters(alert1Config, standardTags());
        assertThat(this.multiplexingAlerter.alerters(),
                is(alerters(filtered(smtp(smtp1Conf, standardTags()), alert1Config.getDuplicateSuppression()))));

        // register an additional http alerter
        HttpAlerterConfig http1Conf = httpConfig("http://hook", ".*");
        List<HttpAlerterConfig> httpAlerters = Arrays.asList(http1Conf);
        AlertersConfig alert2Config = alertConfig(null, httpAlerters, new TimeInterval(18L, TimeUnit.MINUTES));
        this.multiplexingAlerter.registerAlerters(alert2Config, standardTags());
        assertThat(this.multiplexingAlerter.alerters(),
                is(alerters(filtered(smtp(smtp1Conf, standardTags()), alert1Config.getDuplicateSuppression()),
                        filtered(http(http1Conf, standardTags()), alert2Config.getDuplicateSuppression()))));
    }
    ...
    public void dispatch() throws Exception {
        ...
        try {
            ...
            HttpAlerterConfig httpAlerter = httpConfig("http://localhost:" + webServer.getHttpPort(), ".*");
            AlertersConfig alertConfig = alertConfig(asList(smtpAlerter), asList(httpAlerter), null);
            this.multiplexingAlerter.registerAlerters(alertConfig, standardTags());
            ...
            // dispatch alert
            Alert alert = AlertBuilder.create().topic("topic").severity(AlertSeverity.ERROR).message("error!").build();
            String expectedMessage = JsonUtils.toPrettyString(JsonUtils.toJson(alert.withMetadata(standardTags())));
            ...
            assertThat(webServer.getPostedMessages().get(0), is(expectedMessage));
            MimeMessage[] receivedMessages = this.sslMailServer.getReceivedMessages();
            assertThat(receivedMessages.length, is(1));
            SmtpTestServerUtil.assertAlertMail(receivedMessages[0], alert.withMetadata(standardTags()));
        } finally {
            webServer.stop();
        }
    }
    ...
    public void honorDuplicateSuppression() throws Exception {
        ...
        try {
            ...
            SmtpAlerterConfig smtpAlerter = smtpConfig("recipient@company.com", ".*");
            HttpAlerterConfig httpAlerter = httpConfig("http://localhost:" + webServer.getHttpPort(), ".*");
            AlertersConfig alertConfig = alertConfig(asList(smtpAlerter), asList(httpAlerter), duplicateSuppression);
            this.multiplexingAlerter.registerAlerters(alertConfig, standardTags());
            ...
            // dispatch alert
            Alert alert1 = AlertBuilder.create().topic("topic1").severity(AlertSeverity.ERROR).message("error").build();
            Alert alert2 = AlertBuilder.create().topic("topic2").severity(AlertSeverity.INFO).message("info").build();
            ...
            assertAlertMail(this.sslMailServer.getReceivedMessages()[0], "topic1");
            assertAlertMail(this.sslMailServer.getReceivedMessages()[1], "topic2");
            ...
            assertAlertMail(this.sslMailServer.getReceivedMessages()[2], "topic1");
            assertAlertMail(this.sslMailServer.getReceivedMessages()[3], "topic2");
        } finally {
            webServer.stop();
        }
    }
    ...
    private RequestLoggingHttpServer startHttpServer() throws Exception {
        ...
        RequestLoggingHttpServer webServer = new RequestLoggingHttpServer("/", httpPort);
        webServer.start();
        return webServer;
    }
    ...
    private AlertersConfig alertConfig(List<SmtpAlerterConfig> emailAlerters, List<HttpAlerterConfig> httpAlerters,
        ...
        return new AlertersConfig(emailAlerters, httpAlerters, duplicateSuppression);
    }

    private Alerter filtered(Alerter alerter, TimeInterval duplicateSuppression) {
        return new FilteringAlerter(alerter, duplicateSuppression.getTime(), duplicateSuppression.getUnit());
    }

    private Alerter http(HttpAlerterConfig httpAlerterConfig, Map<String, JsonElement> standardTags) {
        return new HttpAlerter(httpAlerterConfig, standardTags);
    }

    private Alerter smtp(SmtpAlerterConfig smtpAlerterConfig, Map<String, JsonElement> standardTags) {
        return new SmtpAlerter(smtpAlerterConfig, standardTags);
    }

    private SmtpAlerterConfig smtpConfig(String recipient, String severityFilter) {
        return new SmtpAlerterConfig(Arrays.asList(recipient), "noreply@elastisys.com", "subject", severityFilter,
                smtpClientConfig());
    }

    private SmtpClientConfig smtpClientConfig() {
        return new SmtpClientConfig("localhost", SMTP_SSL_PORT, new SmtpClientAuthentication(USERNAME, PASSWORD), true);
    }
     ...
     */
    private static void assertAlertMail(MimeMessage emailMessage, String expectedTopic,
