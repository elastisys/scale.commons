...
public class TestSmtpAlerter {
    ...
    public void sendUnauthenticatedNoSsl() throws Exception {
        ...
        SmtpClientConfig noAuthNoSslClient = new SmtpClientConfig("localhost", SMTP_PORT, null, false);
        SmtpAlerter alerter = new SmtpAlerter(new SmtpAlerterConfig(Arrays.asList("recipient@elastisys.com"),
                "sender@elastisys.com", "subject", null, noAuthNoSslClient));
        this.eventBus.register(alerter);

        // post an Alert on the event bus
        Alert alert = new Alert("/alert/topic", AlertSeverity.INFO, UtcTime.now(), "message", null);
    ...
    public void sendAuthenticatedWithSsl() throws Exception {
        ...
        SmtpClientConfig authSslClient = new SmtpClientConfig("localhost", SMTP_SSL_PORT,
    ...
    public void sendWithStandardTags() throws Exception {
        ...
        // post an Alert on the event bus
        this.eventBus.post(new Alert("/alert/topic", AlertSeverity.INFO, UtcTime.now(), "message", null));
        ...
        Alert expectedAlert = AlertBuilder.create().topic("/alert/topic").severity(AlertSeverity.INFO)
                .timestamp(UtcTime.now()).message("message").addMetadata("tag1", "value1")
    ...
    public void suppressAlertWithWrongSeverity() throws Exception {
        ...
        SmtpAlerter alerter = new SmtpAlerter(new SmtpAlerterConfig(Arrays.asList("recipient@elastisys.com"),
                "sender@elastisys.com", "subject", severityFilter, new SmtpClientConfig("localhost", SMTP_SSL_PORT,
                        new SmtpClientAuthentication(USERNAME, PASSWORD), true)));
        this.eventBus.register(alerter);

        // post Alerts with different severity on the event bus
        this.eventBus.post(new Alert("/alert/topic", AlertSeverity.INFO, UtcTime.now(), "info message", null));
        this.eventBus.post(new Alert("/alert/topic", AlertSeverity.WARN, UtcTime.now(), "warn message", null));
        this.eventBus.post(new Alert("/alert/topic", AlertSeverity.ERROR, UtcTime.now(), "error message", null));
        this.eventBus.post(new Alert("/alert/topic", AlertSeverity.FATAL, UtcTime.now(), "fatal message", null));

        // check mailbox after alerts: should only contain alerts that match
        // filter
        MimeMessage[] receivedMessages = this.sslMailServer.getReceivedMessages();
        assertThat(receivedMessages.length, is(2));
        assertThat(extractAlert(receivedMessages[0]).getSeverity(), is(AlertSeverity.ERROR));
        assertThat(extractAlert(receivedMessages[1]).getSeverity(), is(AlertSeverity.FATAL));
    }
