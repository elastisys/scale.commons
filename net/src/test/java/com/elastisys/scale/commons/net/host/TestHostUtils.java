...
public class TestHostUtils {
	...
	public void getHostIpv4AddressesExcludingLoopback() throws IOException {
		...
		assertThat(HostUtils.hostIpv4Addresses(),
				is(HostUtils.hostIpv4Addresses(false)));
	}

	@Test
	public void getHostIpv4AddressesIncludingLoopback() throws SocketException {
		Collection<InetAddress> ipv4Addresses = HostUtils
				.hostIpv4Addresses(true);
		assertThat(ipv4Addresses.isEmpty(), is(false));
		...
		assertTrue(HostUtils.hostIpv4Addresses(true).size() > HostUtils
				.hostIpv4Addresses(false).size());
	}
