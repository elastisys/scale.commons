...
public class TestHttpBuilderServerAuth {
	...
	public void verifyInvalidHostCertWithDefaultTrustStore() throws Exception {
		Server server = startHttpsServer(httpsPort, TRUSTED_SERVER_KEYSTORE,
				TRUSTED_SERVER_KEYSTORE_PASSWORD);

		try {
			Http http = Http.builder().verifyHostCert(true)
	...
	public void verifyValidHostCertWithCustomTrustStore() throws Exception {
		...
		try {
			Http http = Http.builder().verifyHostCert(true)
					...
					.serverAuthTrustStore(KeyStoreType.JKS, CLIENT_TRUST_STORE,
	...
	public void verifyInvalidHostCertWithCustomTrustStore() throws Exception {
		Server server = startHttpsServer(httpsPort, UNTRUSTED_SERVER_KEYSTORE,
				UNTRUSTED_SERVER_KEYSTORE_PASSWORD);
	...
	public void verifyHostName() throws Exception {
		...
		try {
			Http http = Http.builder().verifyHostCert(false)
					.verifyHostname(true).build();
			try {
				http.execute(new HttpGet(url("/")));
	 ...
	 * @return
	 */
	private static Server startHttpsServer(int httpsPort, String keystorePath,
			String keystorePassword) throws Exception {
		ServletDefinition servlet = new ServletDefinition.Builder()
				.servlet(new HelloWorldServlet()).servletPath("/").build();
		Server server = ServletServerBuilder.create().httpsPort(httpsPort)
