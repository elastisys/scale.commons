...
public class TestAuthTokenRequestFilter {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TestAuthTokenRequestFilter.class);

    private static final String SERVER_KEYSTORE_PATH = Resources.getResource("security/server_keystore.p12").toString();
    ...
    public void accessProtectedResourceWithoutAuthToken() throws Exception {
        ...
        assertThat(response.readEntity(ErrorType.class), is(new ErrorType("failed to validate Authorization token",
    ...
    public void accessProtectedResourceWithValidAuthToken() throws Exception {
        ...
        String signedToken = signToken(TOKEN_ISSUER, this.signatureKeyPair, expirationTime);
     ...
     */
    @Test
    public void accessProtectedResourceWithMalformedAuthToken() throws Exception {
        startServer(new AsymmetricKeyAuthTokenValidator(this.signatureKeyPair).withExpectedIssuer(TOKEN_ISSUER));

        String malformedToken = "eyJhbGciOiJIUzI1";
        // access protected resource (with authentication token)
        WebTarget resource = RestClients.httpsNoAuth().target(httpsUrl("/api/protected"));

        String authzHeader = "Bearer " + malformedToken;
        Response response = resource.request().header("Authorization", authzHeader).get();
        assertThat(response.getStatus(), is(Status.UNAUTHORIZED.getStatusCode()));
        assertThat(response.readEntity(ErrorType.class),
                is(new ErrorType("failed to validate Authorization token", "malformed Authorization Bearer token")));
        // verify that error header is present
        assertThat(response.getHeaderString("WWW-Authenticate"), is(notNullValue()));
    }
    ...
    public void accessProtectedResourceWithExpiredAuthToken() throws Exception {
        ...
        DateTime expirationTime = UtcTime.now().plusMinutes(10);
        String tokenWithExpiration = signToken(TOKEN_ISSUER, this.signatureKeyPair, expirationTime);
    ...
    public void failingAuthTokenValidator() throws Exception {
        ...
        String signedToken = signToken(TOKEN_ISSUER, this.signatureKeyPair, UtcTime.now().plusMinutes(10));
        Response response = getWithToken("/api/protected", signedToken);
        assertThat(response.getStatus(), is(Status.UNAUTHORIZED.getStatusCode()));
        assertThat(response.readEntity(ErrorType.class),
                is(new ErrorType("failed to validate Authorization token", "internal error")));
        // verify that error header is present
        assertThat(response.getHeaderString("WWW-Authenticate"), is(notNullValue()));
     ...
     * @throws Exception
     */
    private void startServer(AuthTokenValidator authTokenValidator) throws Exception {
        authTokenFilter = new AuthTokenRequestFilter(authTokenValidator);

        ServletContainer appServlet = new ServletContainer(new SecuredApplication(authTokenFilter));
        ServletDefinition apiServlet = new ServletDefinition.Builder().servlet(appServlet).servletPath("/api")
                .requireHttps(true).requireBasicAuth(false).build();

        this.server = ServletServerBuilder.create().httpsPort(httpsPort).sslKeyStoreType(SslKeyStoreType.PKCS12)
                .sslKeyStorePath(SERVER_KEYSTORE_PATH).sslKeyStorePassword(SERVER_KEYSTORE_PASSWORD)
    ...
    private Response getWithoutToken(String resourcePath) {
        WebTarget resource = RestClients.httpsNoAuth().target(httpsUrl(resourcePath));
