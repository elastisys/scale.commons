...
public class TestJsonValidator {
	...
	private static final String validInstanceWithoutOptionalFields = load(baseDir
			+ "/valid_instance_without_optional_fields.json");
	private static final String validInstanceAllFields = load(baseDir
			+ "/valid_instance_all_fields.json");
	private static final String invalidInstanceNegativeBidPrice = load(baseDir
			+ "/invalid_instance_negative_bidprice.json");
	private static final String invalidInstanceWrongFieldname = load(baseDir
			+ "/invalid_instance_wrong_fieldname.json");
	private static final String invalidInstanceMissingRegion = load(baseDir
			+ "/invalid_instance_missing_region.json");
	private static final String invalidInstanceDisallowedRegion = load(baseDir
			+ "/invalid_instance_disallowed_region.json");
	...
	public void testValidation() throws JsonValidatorException {
		...
		try {
			...
			assertEquals("/properties/awsCredentials/required/2",
					e.getSchemaPath());
		}
		...
		try {
			...
			assertEquals("/properties/awsCredentials/properties/region/type",
					e.getSchemaPath());
		}
	}
