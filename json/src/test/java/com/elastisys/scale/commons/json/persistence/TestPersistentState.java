...
public class TestPersistentState {
	...
	private static final File STORAGE_LOCATION = new File("target",
			"state.json");
	...
	public void firstTimeUse() {
		PersistentState<String> state = new PersistentState<>(STORAGE_LOCATION,
				String.class);
	...
	public void initializeStateStorageOnCreation() {
		...
		PersistentState<Integer> state = new PersistentState<>(
				STORAGE_LOCATION, Integer.class);
	...
	public void storeAndRecoverNumber() {
		PersistentState<Double> state = new PersistentState<>(STORAGE_LOCATION,
				Double.class);
		...
		// recover
		PersistentState<Double> recovered = new PersistentState<>(
				STORAGE_LOCATION, Double.class);
	...
	public void storeAndRecoverString() {
		...
		// recover
		PersistentState<String> recovered = new PersistentState<>(
				STORAGE_LOCATION, String.class);
	...
	public void storeAndRecoverObject() {
		PersistentState<SomeNestedClass> state = new PersistentState<>(
				STORAGE_LOCATION, SomeNestedClass.class);
		...
		SomeNestedClass complexObject = new SomeNestedClass(10, "ten",
				timestamps, new SomeClassWithTimestamp("midnight",
						UtcTime.parse("2015-01-01T00:00:00.000Z")));
		state.update(complexObject);

		// recover
		PersistentState<SomeNestedClass> recovered = new PersistentState<>(
				STORAGE_LOCATION, SomeNestedClass.class);
	...
	public void storeAndRecoverWithGenerics() {
		...
		// recover
		PersistentState<Map<String, DateTime>> recovered = new PersistentState<>(
				STORAGE_LOCATION, stringDateMap);
