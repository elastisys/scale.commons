...
public class CommonFormatLogRecord {
     ...
     * formatter.
     */
    public static final DateTimeFormatter timestampFormatter = DateTimeFormat.forPattern("dd/MMM/yyyy:HH:mm:ss Z")
    ...
    public static CommonFormatLogRecord parse(String logLine) {
        try {
            ...
            String message = String.format("failed to parse apache log line '%s': %s", logLine, e.getMessage());
    ...
    private CommonFormatLogRecord(String logLine) throws Exception {
        ...
        if (matcher.matches()) {
            ...
            this.finishedProcessingTimestamp = UtcTime.parse(matcher.group(4), timestampFormatter);
            ...
            throw new IllegalArgumentException(
                    "input string: '" + logLine + "' did not match expression '" + LOG_ENTRY_REGEXP + "'");
        }
    }
